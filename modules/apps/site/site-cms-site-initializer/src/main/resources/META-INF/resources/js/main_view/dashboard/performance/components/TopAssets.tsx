...
// DON'T MERGE - Temporary data source. The FDS-compatible analytics endpoint is
// not ready yet, so we feed the table with the "All" section endpoint (the
// created assets) to mirror the final server-backed behavior (real
// search/order/pagination). The metric columns (Trend, Engagement Rate,
// Impressions, Views, Downloads) stay empty until the real endpoint lands; then
// only the apiURL and the field mapping change.

const API_URL = '/o/search/v1.0/search';

const ADDITIONAL_API_URL_PARAMETERS =
	"emptySearch=true&filter=(cmsSection eq 'contents' or cmsSection eq " +
	"'files') and rootDescendantNode eq false&nestedFields=embedded," +
	'embeddedTaxonomyCategory,file.metadata,file.previewURL,' +
	'file.thumbnailURL,modifiedBy,numberOfObjectEntries,' +
	'numberOfObjectEntryFolders,systemProperties.collaboratorBrief,' +
...
export function TopAssets() {
	...
	const infoPanelComponent = useMemo(() => {
		...
		return ({items = []}: IInfoPanelComponent) => (
			<AssetTypeInfoPanel
